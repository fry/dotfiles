...
;; $Revision: 1.12 $ $Date: 2004-07-31 16:39:25 $ $Author: eric $

;; Author: Eric Engstrom <engstrom (AT) m t u (DOT) n e t>
;;    [ email address can be put back together in the human obvious fashion ]
;; Maintainer: Eric Engstrom
;; Keywords: spin, promela, tools

;; Copyright (C) 1998-2004  Eric Engstrom 

;; ... Probably insert GPL here someday ...
...
;;      Note that promela-mode adheres to the font-lock "standards" and
...
;; Please send any comments, bugs, patches or other requests to the author.
...
;;   added error-regexp for compilation mode - slightly better now..
...
(defconst promela-mode-version "$Revision: 1.12 $"
...
;;	 Pat Tullman (tullmann(AT)cs(DOT)utah(DOT)edu) and
;;	 Ny Aina Razermera Mamy (ainarazr(AT)cs(DOT)uoregon(DOT)edu)
...
(defconst promela-error-regexp-alist
  '(("spin: +\\(warning, +\\)?line +\\([0-9]+\\) +\"\\([^\"]+\\)\"" 3 2))
  "Regexp describing syntax errors from spin.")
...
(if promela-mode-map
  ...
  (define-key promela-mode-map "\C-c\C-c"	'promela-syntax-check)
  (define-key promela-mode-map "\C-c\C-s"	'promela-run-simulator)
  (define-key promela-mode-map "\C-c\C-g"	'promela-generate-pan)
  (define-key promela-mode-map "\C-c\C-b"	'promela-generate-pan)
  (define-key promela-mode-map "\C-c\C-v"	'promela-run-verifier)
  ;;
...
"
  ...
  (setq case-fold-search 		t
	...
	;;comment-start-skip 		"/\\*+ *\\|// *"
        ...
        compilation-error-regexp-alist	promela-error-regexp-alist
        )
...
(setq  
 ;; run spin to do syntax check and generate pan.c, etc.
 promela-spin-command		"spin "
 promela-spin-generate-args	"-a -v "
 ;; run the compiler to compile pan.c to 
 promela-compile-command	"gcc "
 promela-compile-pan-args	"-o pan "
 promela-compile-user-args	"-DSAFETY -DNOFAIR "
 promela-compile-pan-file	"pan.c"
 ;; run pan for verification
 promela-pan-command		"pan "
 promela-pan-args		""
)

;;(defun promela-syntax-check ()
;;  (interactive)
;;  (comint-exec (get-buffer-create "*spin*")
;;               "*spin*" 
;;               promela-spin-command
;;               ""
;;               (list 
;;                       promela-spin-generate-args
;;                       (buffer-name)))

(defun promela-syntax-check ()
  (interactive)
  (compile (concat promela-spin-command " "
                   promela-spin-generate-args " "
                   (buffer-name)))
)

(defun promela-run-simulator ()
  (interactive)
  (message "not done yet...")
)

(defun promela-compile-pan ()
  (interactive)
  (compile (concat promela-compile-command " "
                   promela-compile-pan-args " "
                   promela-compile-user-args " "
                   promela-compile-pan-file))
)

(defun promela-run-verifier ()
  (interactive)
  (promela-syntax-check)
  (promela-compile-pan)
  (compile (concat promela-pan-command 
                   promela-pan-args))
)

;; -------------------------------------------------------------------------
